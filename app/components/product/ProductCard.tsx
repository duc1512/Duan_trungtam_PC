...
const getMockRating = (productId: string): { rating: number; count: number } => {
  const hash = productId.split("").reduce((acc, char) => acc + char.charCodeAt(0), 0);
  return {
...
const ProductCard = memo(({ 
  ...
  const { rating, count: ratingCount } = useMemo(() => getMockRating(product.id), [product.id]);
  ...
  return (
    ...
    >
      <div className={`bg-white border border-gray-100 overflow-hidden hover:shadow-2xl hover:shadow-gray-200/50 hover:-translate-y-1 transition-all duration-300 ${
        isFlashSale ? "rounded-xl" : "rounded-xl"
      }`}>
        {/* Image Container */}
        <div className={`relative bg-gray-50 overflow-hidden ${isCompact ? "aspect-[4/3]" : "aspect-square"}`}>
          {/* Loading Skeleton */}
          ...
          {showBadges && discount > 0 && (
            <div className="absolute top-2 left-2 z-10">
              <div className={`bg-gradient-to-r from-red-500 to-red-600 text-white font-bold rounded shadow-md ${
                isCompact ? "text-[9px] px-1.5 py-0.5" : "text-[10px] px-2 py-1"
              }`}>
                -{discount}%
              </div>
            </div>
          ...
          {/* Rank Badge */}
          {showRank && index < 3 && (
            <div className={`absolute top-2 right-2 z-10 rounded-full flex items-center justify-center font-bold shadow-md ${
          ...
          {showBadges && soldCount > 5000 && (
            <div className="absolute bottom-2 left-2 z-10">
              <div className={`bg-gradient-to-r from-orange-500 to-red-500 text-white font-bold rounded flex items-center gap-0.5 ${
                isCompact ? "text-[8px] px-1.5 py-0.5" : "text-[9px] px-2 py-0.5"
              }`}>
                <svg className={isCompact ? "w-2 h-2" : "w-3 h-3"} fill="currentColor" viewBox="0 0 20 20">
          ...
          >
            <svg className={isCompact ? "w-3.5 h-3.5" : "w-4 h-4"} fill={isWishlisted ? "currentColor" : "none"} stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div className={isCompact ? "p-2" : "p-3"}>
          ...
          {/* Product Name */}
          <h3 className={`text-gray-800 font-medium line-clamp-2 leading-snug group-hover:text-red-600 transition-colors ${
          ...
          <div className={`flex items-baseline gap-2 ${isCompact ? "mt-1" : "mt-2"}`}>
            <span className={`text-red-600 font-bold ${isCompact ? "text-sm" : "text-base"}`}>
          ...
          <div className={`flex items-center justify-between ${isCompact ? "mt-1" : "mt-2"}`}>
            ...
            {soldCount > 8000 && (
              <span className={`text-red-500 font-medium animate-pulse ${isCompact ? "text-[9px]" : "text-[10px]"}`}>
                🔥 Sắp hết
              </span>
            )}
          </div>
            ...
            <div className="mt-2">
              <div className="h-1.5 bg-gray-100 rounded-full overflow-hidden">
                <div 
                  ...
                  style={{ width: `${Math.min(95, (soldCount / 10000) * 100)}%` }}
                />
              </div>
            </div>
          )}
